...
# Identify super matchups
super_matchups = combined_df[
    (combined_df['TEAM_Merge'].isin(draft_df['School_Merge'])) &
    (combined_df['TEAM_Merge'].isin(draft_df['School_Merge']))
]
super_matchups = super_matchups[['AWAY', 'HOME', 'DATE', 'TIME (ET)']].drop_duplicates()
...
with st.expander("More Information", expanded=False):
    st.text("This page helps basketball fans keep track of upcoming NCAA games featuring top prospects for the 2025 NBA Draft. If you don’t follow college basketball but want to know when the next potential stars are playing, this is your go-to schedule.")
...
st.dataframe(super_matchups, hide_index=True)

# Date-specific filtering
selected_date = st.date_input("Select Date", min_value=date.today(), max_value=date.today() + timedelta(days=7))
st.header(f"Schedule for {selected_date}")
st.dataframe(combined_df[combined_df['DATE'] == selected_date], hide_index=True)
